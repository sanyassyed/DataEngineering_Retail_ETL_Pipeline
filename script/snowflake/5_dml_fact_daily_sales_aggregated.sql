...
------------------------------------------------------
-- This will run **DAILY**
-- Creates sales fact with the sales aggregated to a day.

-- Getting Last Date
SET LAST_SOLD_DATE_SK = (SELECT MAX(SOLD_DATE_SK) FROM SF_TPCDS.INTERMEDIATE.fact_daily_sales_aggregated);

-- Removing partial records from the last date
DELETE FROM SF_TPCDS.INTERMEDIATE.fact_daily_sales_aggregated WHERE sold_date_sk=$LAST_SOLD_DATE_SK;



CREATE OR REPLACE TEMPORARY TABLE SF_TPCDS.INTERMEDIATE.fact_daily_sales_aggregated_TMP AS (
...
SELECT 
    ...
    from sf_tpcds.raw.catalog_sales
...
LEFT JOIN sf_tpcds.raw.date_dim date 
    ON sold_date_sk = d_date_sk

)

SELECT -- doing this just as a second safety net
	warehouse_sk,
    item_sk,
    sold_date_sk,
...
INSERT INTO SF_TPCDS.INTERMEDIATE.fact_daily_sales_aggregated
(	
    WAREHOUSE_SK, 
    ITEM_SK, 
    SOLD_DATE_SK, 
...
SELECT 
    ...
    daily_sales_amt,
    daily_net_profit 
FROM SF_TPCDS.INTERMEDIATE.fact_daily_sales_aggregated_TMP;