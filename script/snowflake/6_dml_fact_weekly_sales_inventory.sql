-----------------------------------------------------
-- fact_weekly_sales_aggregated
------------------------------------------------------

-- Creates sales fact with the sales aggregated to a week.
-- This will run **WEEKLY**

-- Getting Last Date
SET LAST_SOLD_WK_SK = (SELECT MAX(SOLD_WK_SK) FROM SF_TPCDS.ANALYTICS.fact_weekly_sales_inventory);

-- Removing partial records from the last date
DELETE FROM SF_TPCDS.ANALYTICS.fact_weekly_sales_inventory WHERE sold_wk_sk=$LAST_SOLD_WK_SK;

-- compiling all incremental sales records
CREATE OR REPLACE TEMPORARY TABLE SF_TPCDS.ANALYTICS.fact_weekly_sales_inventory_TMP AS (
...
FROM
    SF_TPCDS.INTERMEDIATE.fact_daily_sales_aggregated
GROUP BY
    1,2,4,5
HAVING 
...
INNER JOIN SF_TPCDS.RAW.DATE_DIM as date
...
FROM
    sf_tpcds.raw.inventory inventory
INNER JOIN SF_TPCDS.RAW.DATE_DIM as date
...
INSERT INTO SF_TPCDS.ANALYTICS.fact_weekly_sales_inventory
(	
	WAREHOUSE_SK, 
    ITEM_SK, 
    SOLD_WK_SK, 
...
SELECT 
    ...
    WKS_SPLY, 
    LOW_STOCK_FLG_WK
FROM SF_TPCDS.ANALYTICS.fact_weekly_sales_inventory_TMP;

select * from SF_TPCDS.ANALYTICS.fact_weekly_sales_inventory;